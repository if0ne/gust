...
A `ResolvedComponent` (`component.rs`) is produced in two steps: `UnresolvedComponent::new` compiles the bytes and extracts the component's WIT world (imports/exports, modeled in `wit.rs`) from its `component_type()`; `.resolve(extensions)` then freezes the linker and yields an `Arc`-backed, cloneable `ResolvedComponent`. Instantiation goes through `instantiate_pre()` → `CommandPre` so the pre-linked component can be reused. The store data is `Context` (`ctx.rs`), which implements both `WasiView` and `WasiHttpView`.
...
- `src/service/runtime/` — the WASM runtime: `engine` (shared `Engine` + `EpochTicker` + pooling-allocator detection), `cache` (moka compiled-component cache), `runner` (`Runner` registry + `RunnerBuilder`), `component` (`UnresolvedComponent`/`ResolvedComponent`), `ctx` (`Context`/`ContextBuilder` — `WasiView` + `WasiHttpView`), `common` (`ComponentId`/`ExtensionId`/`ComponentSpec`/`InterfaceConfigMap`), `image` (`ImageRef`), `wit` (`WitInterface`/`WitWorld`), `extension` (`Extension` trait — scaffolding), `resolver` (`Resolver` trait + `DefaultResolver`)
